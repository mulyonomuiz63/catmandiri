...
export default {
    ...
    setup(props) {
        const form = reactive({
            ...
            member_categories: !props.videoModule.member_categories
                ? []
                : props.videoModule.member_categories.map((item) => {
                      return { name: item };
                  }),
        });
        ...
        const submit = () => {
            // send data to server
            Inertia.put(
                `/admin/video-modules/${props.videoModule.id}`,
                {
                    // data
