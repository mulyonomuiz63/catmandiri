    ...
    public function examStart($id, Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()->member_type == 2) {
                ...
                if (!$checkExam) {
                    throw new \Exception('Exam group not found');
                }

                $checkMemberCategories = $this->checkMemberCategories(
                    UserMemberCategory::where(['category_id' => $exam->category_id, 'user_id' => Auth::user()->id])
                        ->where('expired_date', '>', Carbon::now())
                        ->get(),
                    $checkExam->member_categories
                );

                if ($setting->purchase_type == 1 && count($checkExam->transaction) > 0) {
                    $checkAcess = true;
                } elseif ($setting->purchase_type == 2 && $checkMemberCategories == true) {
                    $checkAcess = true;
                } elseif ($setting->purchase_type == 3 && (count($checkExam->transaction) > 0 || $checkMemberCategories == true)) {
                    $checkAcess = true;
                } else {
                    $checkAcess = false;
                }
